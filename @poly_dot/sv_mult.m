...
%SV_MULT dot product for the POLY dot
%
...
elseif nargin == 3
  ...
  for i = 1:num_blocks
	...
	(d.scale * a(:,lower_limit:upper_limit)' * a + d.offset).^d.degree * b;
    lower_limit = upper_limit + 1;
  end;
  if (lower_limit <= num_dims)
    x(lower_limit:num_dims,:) = ...
	(d.scale * a(:,lower_limit:num_dims)' * a + d.offset).^d.degree * b;
  end;
else
  num_dims = size(a,2);
  num_blocks = floor(num_dims / d.blocksize);
  ...
  for i = 1:num_blocks
    upper_limit = upper_limit + d.blocksize;
    x(lower_limit:upper_limit,:) = ...
	(d.scale * a(:,lower_limit:upper_limit)' * b + d.offset).^d.degree * c;
    lower_limit = upper_limit + 1;
  end;
  if (lower_limit <= num_dims)
    x(lower_limit:num_dims,:) = ...
	(d.scale * a(:,lower_limit:num_dims)' * b + d.offset).^d.degree * c;
  end;
end;
