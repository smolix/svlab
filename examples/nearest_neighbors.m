...
knn_obj.numneighbors = 20;  %get the 20 nearest neighbors to each test pattern
...
nearest_test = nearest(knn_obj,ker,training_patterns,test_patterns);

% use the index returned by 'nearest' to get a matrix of nearest neighbors
% for each of the first two test points
nearest_1 = training_patterns(:,nearest_test(:,1)');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% plot the results
figure(1)
plot(training_patterns(1,1:cluster_size),training_patterns(2,1:cluster_size),'bx')
hold on
plot(training_patterns(1,cluster_size+1:2*cluster_size),training_patterns(2,cluster_size+1:2*cluster_size),'b.')
