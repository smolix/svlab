function [T,basisvecsindex,error_sequence,delta_sequence] = sgma_train(sgma_var,ker, X)
% function[T,basisvecsindex,error_sequence,delta_sequence] = sgma_train(sgma_var,ker, X)
%
% A modified version of @sgma/train.m which also returns the sequence
% of errors and deltas so they can be plotted. 
% Used by the test file examples/sgma_example.m
%
...
while ((approx_error > error_bound)... 
	...
	randomindex=randperm(numvecsleft);
