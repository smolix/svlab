function [phi d] = train(d, kernel, x, lambda, nu)
...
if(~d.dim)
  ...
  if (buffer_is_not_full)
    phi = sv_mult(kernel, x, d.online_data(:,1:online_stop), ...
		d.online_alpha(1:online_stop)) - d.online_rho;
  else
  ...
  if (phi < 0)
    ...
    d.online_alpha(d.online_stop) = lambda;
    d.online_data(:,d.online_stop) = x;
    d.online_rho = d.online_rho + lambda * (nu-1);
  else
    d.online_rho = d.online_rho + (lambda * nu);
  end
